function [data, dataInfo, info] = UTESdate(data, dataInfo, info)
...
for i = 1:length(data)
    ...
    if ~isempty(data{i})
        ...
        if isempty(row)
            dataInfo{end+1,i} = strcat('beg date:',datestr(UTESdate(1)));
            dataInfo{end+1,i} = strcat('end date:',datestr(UTESdate(end)));
        else
            dataInfo{row,i} = strcat('beg date:',datestr(UTESdate(1)));
            dataInfo{row+1,i} = strcat('end date:',datestr(UTESdate(end)));
        end
        
        info.date = datestr(UTESdate(1),'yyyy_mm_dd');
